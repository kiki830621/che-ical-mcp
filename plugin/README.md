...
- **Identity-guarded undo for recurring completions (#204)**: undo/redo act only while the identifier still resolves to the recorded occurrence; otherwise they refuse explicitly and drop the entry so older operations stay undoable. Follow-up: #205 (strict boolean `completed`, BREAKING, PR #201).
