...
7. recurring undo/redo 以身分快照 guard（PR #200 / #204）：可辨識的 recurring 快照記錄 `.completeRecurringReminder`，undo/redo 前 refresh 並比對同 occurrence；身分已移動 → 明確失敗並**丟棄**紀錄（不塞回 stack）；找不到 → transient、保留紀錄；不可辨識的快照退回既有 `.completeReminder`。
