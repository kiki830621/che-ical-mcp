...
Undo/redo of a recurring completion is identity-guarded: the record keeps the pre-write due/rules/list/source snapshot, and the operation runs only if the identifier still resolves to that same occurrence (completion state and title are not identity). Once the identifier has advanced to a later occurrence — on iCloud the finished occurrence is kept as a separate completed record — the undo fails with an explicit message and the history entry is **discarded**, so every earlier operation stays undoable; reopen the finished record explicitly instead. Transient failures (permission, store errors, a reminder that cannot be found right now) still keep the entry for a retry (#191). A recurring snapshot that could never be matched again (no comparable due, no known rules, an unknown frequency) is recorded with the legacy identifier-keyed entry instead, so an unchanged item stays undoable. The guard refreshes the store before comparing. Non-recurring undo is unchanged; `undo_history` lists guarded entries as `Completed recurring reminder: <title>` / `Reopened recurring reminder: <title>`.
