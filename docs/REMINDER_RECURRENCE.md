...
Undo/redo of a recurring completion is identity-guarded: the record keeps the pre-write due/rules/list/source snapshot, and the operation runs only if the identifier still resolves to that same occurrence (completion state and title are not identity). Once the identifier no longer resolves to the recorded occurrence — the series advanced (on iCloud the finished occurrence is kept as a separate completed record), or the item's due / rules / list / source were edited since — the undo fails with an explicit message and the history entry is **discarded**, so every earlier operation stays undoable; act on the intended occurrence explicitly instead. Transient failures (store errors thrown by EventKit) keep the entry for a retry (#191). A reminder that a store fetch cannot find is fetched once more after `refreshSourcesIfNecessary()` (a non-blocking hint, not a synchronous pull); if it is still missing the entry is **discarded** with a message that names the entry and its identifier (#206), so a dead entry no longer jams every older entry behind an always-failing head. EventKit cannot positively confirm a deletion: a reminder moved to another account (which assigns a new identifier) or whose account is temporarily unavailable is dropped the same way, and the message says so instead of asserting deletion. Exactly five reminder history arms behave this way — the `.createReminder`, `.updateReminder` and `.completeReminder` undo arms, the `.completeReminder` redo arm, and the recurring completion undo/redo resolver; event arms are unchanged. A recurring snapshot with no due or no rules is recorded with the legacy identifier-keyed entry instead — for that subset there is still no identity guard: undo and redo act on whatever the identifier now resolves to (after a rollover, undo writes the recorded state and instant onto the successor; redo applies the recorded request to it). The guard compares against a fresh store fetch of the item (#206), not the cache this completion itself marked dirty. A redo never re-snapshots: after a redo that rolled the series over, the next undo fails and is discarded. On-device (iCloud, 2026-09-07): after a confirmed rollover the undo refused with the message above and the older entry became undoable; with no rollover (`occurrence_count: 1`) undo and redo both succeeded. Non-recurring undo restores the recorded completion instant (#196) and is otherwise unchanged; `undo_history` lists guarded entries as `Completed recurring reminder: <title>` / `Reopened recurring reminder: <title>`. Every undo that returns a reminder to the completed state — the completion records and the update / delete records alike — writes the recorded completion instant back instead of letting EventKit stamp the undo time (#196); redo re-applies the recorded request (a redo that completes stamps a fresh instant). The mapping from each record to its write (`UndoOperation.completionWrite`), the write itself (`ReminderCompletionWrite.apply`, on an in-memory `EKReminder`) and both snapshot captures are unit-tested; a reverted mapping fails the suite. The save round-trip has not yet been exercised on device — stateful undo needs the session MCP surface after a restart.
