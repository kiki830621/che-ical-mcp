...
class CheICalMCPServer {
                        ...
                        "recurrence": .object([
                            ...
                            "properties": .object([
                                ...
                                "excluded_occurrence_dates": .object([
                                    "type": .string("array"),
                                    "items": .object(["type": .string("string")]),
                                    "description": .string("Occurrence dates to exclude from the series at creation (max 100; same date grammar as occurrence_date - date-only values use the event timezone). Applied atomically with series creation: on any failure the new series is rolled back. Each date must resolve to an occurrence generated by the rule. Only supported here and on create_events_batch.")
                                ])
                            ]),
                                ...
                                "properties": .object([
                                    ...
                                    "recurrence": .object([
                                        ...
                                        "properties": .object([
                                            ...
                                            "excluded_occurrence_dates": .object(["type": .string("array"), "items": .object(["type": .string("string")]), "description": .string("Occurrence dates to exclude at creation (max 100, occurrence_date grammar, atomic with rollback)")])
                                        ]),
                                        "required": .array([.string("frequency")])
                                    ]),
    ...
    private func handleCreateEvent(arguments: [String: Value]) async throws -> String {
        ...
        var payload: [String: Any] = ["action": "created", "title": result.event.title ?? title, "id": result.event.eventIdentifier ?? "unknown"]
        // #182 — response contract: normalized excluded days + count.
        if let excluded = recurrenceRule?.excludedOccurrenceDates, !excluded.isEmpty {
    ...
    private func handleCreateEventsBatch(arguments: [String: Value]) async throws -> String {
        ...
        for (index, eventValue) in eventsArray.enumerated() {
            ...
            do {
                ...
                // #182 — per-item deterministic exclusion reporting.
                if !result.isDuplicate, let excluded = batchRecurrence?.excludedOccurrenceDates, !excluded.isEmpty {
    ...
    private func parseRecurrenceRule(from arguments: [String: Value], defaultTimezone: TimeZone? = nil, allowsExclusions: Bool = false) throws -> RecurrenceRuleInput? {
        ...
        if let excludedValue = recurrenceDict["excluded_occurrence_dates"] {
            ...
            // Explicit empty array is the right type with no work to do — safe no-op.
            excludedDates = parsed.isEmpty ? nil : parsed
        }
