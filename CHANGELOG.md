...
- **A dead reminder history entry no longer jams the undo stack** (#206): the five reminder history arms (`.createReminder` / `.updateReminder` / `.completeReminder` undo, `.completeReminder` redo, recurring completion undo/redo) resolve their target with a real store fetch (not the in-memory cache — a stale hit could otherwise mask an external deletion), fetch once more after `refreshSourcesIfNecessary()`, and on a second miss throw the permanent `UnrecoverableUndoError` so the entry is discarded (`UndoFailureDisposition.discard`) and the rest of the history stays reachable. The message names the entry and identifier and lists what a miss can mean — deleted, moved to another account (new identifier), or account temporarily unavailable — because EventKit cannot confirm deletion. Store errors remain transient and keep the entry. `.createReminder` undo no longer reports success when the reminder is already gone (Refs #206).
...
**#205 — `completed` must be a JSON boolean on the reminder tools (BREAKING).**

- **BREAKING** — `completed` must be a JSON boolean on `complete_reminder`, `list_reminders` and `search_reminders`. A string or number (`"false"`, `"true"`, `0`, `1`) is now rejected with `Invalid parameter: completed must be a boolean (true or false)` before any read or write — on `list_reminders` even when `filter` is supplied; omitted and JSON `null` keep their old meaning (complete / no filter). Previously `complete_reminder` silently treated any non-boolean as `true` — a caller sending `"completed": "false"` to reopen a reminder completed it instead — while list/search silently treated it as "no filter". Migration: send `true` / `false` literals; loosely typed clients that stringify booleans must stop doing so for this parameter. The `--cli` stdin surface now maps JSON `null` to an omitted argument instead of `""` (Refs #205).
